...
def json_handler(jsons_path, pad_length):
    ...
    used as input to Multi-Layer Perceptron (MLP) model alternative.
    ...
    return np.concatenate((sequence1, sequence2, sequence3), axis = 1)
...
def neural_network(num_labels, pad_length):
    ...
    concatenated. This sequence is entered in a Multi-layer Perceptron architecture.
    ...
    # Create the model making use of Sequential()
    model = Sequential()

    # Dense() is a fully-connected layer with a certain number hidden units.
    # in the first layer, you must specify the expected input data shape.
    # Not necessary to specify the number of input dimensions for deeper layers.
    model.add(Dense(units=250, activation='relu', input_dim=3*pad_length))
    # Dropout layer after input layer
...
def save_checkpoint(filepath):
    ...
    checkpoint = ModelCheckpoint(filepath='weights-mlp-epoch-{epoch:02d}-val_acc-{val_acc:.2f}.hdf5',
                                 monitor='val_acc',
                                 verbose=1,
                                 save_best_only=True,
                                 mode='max')
...
def model_evaluate(model, inputs, labels, verbose):
    ...
    """
    scores = model.evaluate(x=inputs, y=labels, batch_size=1, verbose=verbose)
...
class Agent:
    ...
    def run(self):
        ...
        if self.training:
            print ('Training of MLP model starting')
            ...
            if self.save_graph:
                ...
                graph_model(model=self.model, graph_name='mlp-model', filepath=self.graph_dir)
        ...
        # following steps will be taken
            ...
            print('Evaluation of MLP model starting')
            ...
            self.checkpoint_name = max(glob.glob(self.checkpoint_base_dir + '*mlp*'),
